...
class BinaryAttentionBias(nn.Module):
    ...
    def forward(self, query_id, kv_id):
        ...
        ind = ind.unsqueeze(1)
        weight = self.emd.weight.view(2, self.num_heads, 1, 1)
    ...
    def __init__(self, d_model=2048, d_proj=512, patch_size=32, num_layers=6, num_heads=8,
        ...
        if use_rope:
            ...
            self.transformer_encoder = CustomTransformerEncoder(
                d_model, num_heads, d_model * 4, d_ff_dropout, activation, num_layers, num_features
            )
        else:
            encoder_layer = nn.TransformerEncoderLayer(
                d_model, num_heads, d_model * 4, d_ff_dropout, batch_first=True, activation=activation
            )
...
class FloodScout(nn.Module):
    """
    Lightweight 1D-CNN that predicts flood probability from history only.
    
    This is "Module A" (The Scout) that analyzes history and outputs a
    flood probability to condition the main TimeRCD model.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: (B, context_len, 1) - history sequence
        Returns:
...
class TimeRCDWithPrior(nn.Module):
    """
    TimeRCD model conditioned on FloodScout's prior token.
    
    This combines:
    1. FloodScout (frozen) - analyzes history -> flood probability
    2. PriorEmbedding - converts probability to token
    3. TimeSeriesPretrainModel - generates predictions conditioned on token
    ...
    def __init__(self, config: TimeRCDConfig, scout_checkpoint: Optional[str] = None, context_len: int = 336):
        ...
        # Load scout weights if provided
        if scout_checkpoint and os.path.exists(scout_checkpoint):
            print(f"Loading FloodScout weights from {scout_checkpoint}")
            scout_state = torch.load(scout_checkpoint, map_location='cpu')
            if 'model_state_dict' in scout_state:
                scout_state = scout_state['model_state_dict']
            self.scout.load_state_dict(scout_state, strict=True)
        
        # Expose reconstruction_head for compatibility
        self.reconstruction_head = self.timercd.reconstruction_head
    
    def freeze_scout(self):
        """Freeze FloodScout parameters (call before Phase 2 training)."""
        for param in self.scout.parameters():
            param.requires_grad = False
        self.scout.eval()
        print("FloodScout frozen.")
    
    def unfreeze_scout(self):
        """Unfreeze FloodScout parameters (if needed for joint training)."""
        for param in self.scout.parameters():
            param.requires_grad = True
        self.scout.train()
        print("FloodScout unfrozen.")
    ...
    def forward(self, time_series: torch.Tensor, mask: torch.Tensor, 
        ...
        Args:
            ...
            mask: (B, seq_len) - attention mask
            flood_prob: Optional (B, 1) - pre-computed flood probability
                        If None, computed from history using FloodScout
    ...
    def get_flood_probability(self, history: torch.Tensor) -> torch.Tensor:
        """Get flood probability from history only (for inference)."""
        return self.scout(history)


# ==========================================
# 5. Dataset (FloodDataset - Univariate)
# ==========================================
...
class IngestionDataset(Dataset):
    def __init__(self, train_csv, test_csv, test_index_csv, metadata_path, context_len=336, pred_len=336):
        ...
        for name, group in self.full_df.groupby('station_name'):
            ...
            if meta:
                ...
                self.station_data[name] = {'values': norm_vals, 'times': group.index}
            else:
                std = np.std(vals)
                estimated_thresh = np.mean(vals) + 2.29 * std
                norm_vals = (vals - estimated_thresh) / std
                self.station_data[name] = {'values': norm_vals, 'times': group.index}
                print(f"Warning: No metadata for {name}, using estimated threshold: {estimated_thresh}")
    ...
    def __getitem__(self, idx):
        ...
        return {'time_series': full_seq, 'mask': mask, 'id': row['id'], 'station_name': station_name}


def ingestion_predict(args, use_prior=False):
    """
    Ingestion prediction with optional Prior Token model.
    ...
    if use_prior:
        ...
        model = TimeRCDWithPrior(config, scout_checkpoint=scout_checkpoint).to(device)
        model.freeze_scout()
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scout_checkpoint", type=str, default="scout.pkl", help="Path to FloodScout checkpoint")
    ...
    if args.train_hourly and args.test_index:
        ingestion_predict(args, use_prior=args.use_prior)
